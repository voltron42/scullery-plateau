...
(defn- default-ops {'entry-set (fn [my-map] (mapv (fn [[k v]] {:k k :v v}) (sorted-map my-map)))})
    ...
    (list? tpl) (let [[op & args] tpl]
                  (cond
                    (map? op) (let [[label col] (first op)
                                    col (if (keyword? col) (data col) col)
                                    col (if (list? col) (resolve-tpl op-map data [] col) col)
...
(defn build-template-factory [op-map]
  (fn build-template [tpl]
    (fn resolve-template [data]
      (resolve-tpl (merge op-map default-ops) data [] tpl))))
